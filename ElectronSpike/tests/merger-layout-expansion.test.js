...
test("merger page lets conflict lists grow downward up to a max height before scrolling", () => {
    ...
    assert.match(css, /body\[data-active-tab="merger"\]\s*\{[\s\S]*overflow:\s*auto;/);
    assert.match(css, /body\[data-active-tab="merger"\]\s+\.window-shell\s*\{[\s\S]*height:\s*auto;[\s\S]*grid-template-rows:\s*auto auto auto;/);
    assert.match(css, /body\[data-active-tab="merger"\]\s+\.content\s*\{[\s\S]*overflow:\s*visible;[\s\S]*grid-template-rows:\s*auto;/);
    assert.match(css, /body\[data-active-tab="merger"\]\s+\.page-section\.hidden\s*\{[\s\S]*display:\s*none !important;/);
    assert.match(css, /\.page-section-merger\s*\{[\s\S]*overflow:\s*visible;[\s\S]*grid-template-rows:\s*auto auto auto auto;/);
